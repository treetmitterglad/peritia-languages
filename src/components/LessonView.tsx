...
import { LessonData, LessonExercise, LanguageCode, Difficulty, SUPPORTED_LANGUAGES } from "@/lib/types";
import { generateLesson } from "@/lib/mistral";
import { getApiKey, getProgress, saveProgress } from "@/lib/storage";
import { Loader2, Check, X, ArrowRight, Zap, Trophy } from "lucide-react";
...
const LessonView = ({ language, difficulty, onBack }: LessonViewProps) => {
      ...
      >
        ...
        {lesson.newWords && lesson.newWords.length > 0 && (
          <div className="bg-card border border-border rounded-xl p-4 w-full max-w-sm">
            <p className="text-sm font-semibold text-foreground mb-2">New words learned:</p>
        ...
        <div className="flex gap-3">
          <button onClick={onBack} className="px-6 py-3 rounded-xl border border-border bg-card text-foreground font-medium hover:bg-secondary transition-colors">
            Home
          </button>
          <button onClick={loadLesson} className="px-6 py-3 rounded-xl bg-primary text-primary-foreground font-medium hover:opacity-90 transition-all">
            Next Lesson →
          </button>
        </div>
      </motion.div>
  ...
  const progressPct = lesson ? ((currentIdx) / lesson.exercises.length) * 100 : 0;
        ...
        >
          ...
          {answered && exercise.explanation && (
            <motion.p
              ...
              className="mt-4 p-4 rounded-xl bg-secondary text-secondary-foreground text-sm"
            >
      ...
      <AnimatePresence>
        {answered && (
          ...
          >
            ...
            >
              {currentIdx < (lesson?.exercises.length ?? 0) - 1 ? (
                <>
